...
@Client.on_message(filters.command("mgconcept"))
async def mgconcept_handler(client: Client, m: Message):
    ...
    courses = requests.get(
        f"https://mgconceptapi.classx.co.in/get/mycourse?userid={userid}",
    ...
    topics = requests.get(
        f"https://mgconceptapi.classx.co.in/get/alltopicfrmlivecourseclass?courseid={batch_id}&subjectid={subject_id}",
        headers=auth_headers,
    ).json()["data"]
    ...
    filename = "MgConcept-Institute.txt"
    ...
    for tid in selected_topics:
        ...
        res = requests.get(
            "https://mgconceptapi.classx.co.in/get/livecourseclassbycoursesubtopconceptapiv3",
            params={
                "topicid": tid,
                "start": -1,
                "conceptid": 1,
